...
import { 
  ...
  Clock, ChefHat, BookOpen, ChevronLeft, ChevronRight
} from 'lucide-react';
import { 
    ...
    <div className="min-h-screen pb-32">
      ...
      <div className="container mx-auto px-4 -mt-12 relative z-20">
        ...
        <div className="flex overflow-x-auto pb-8 gap-3 no-scrollbar justify-center">
          <div className="flex bg-white/90 backdrop-blur-2xl p-3 rounded-full border shadow-2xl">
            {CATEGORIES.map(cat => (
              <button
                key={cat}
        ...
        <div className="mt-20">
          ...
          >
            <CarouselContent className="-ml-6">
              {filteredItems.map(item => (
                <CarouselItem key={item.id} className="pl-6 md:basis-1/2 lg:basis-1/3 xl:basis-1/4">
                  ...
                  >
                    <div className="bg-white rounded-[3rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-700 border border-muted h-full flex flex-col">
                      <div className="relative aspect-[4/5] overflow-hidden">
                        ...
                        <Badge className="absolute top-8 right-8 bg-white/95 text-primary border-none shadow-2xl px-6 py-3 font-black text-xl rounded-[1.5rem]">
            ...
            <div className="hidden md:flex justify-center gap-6 mt-16">
              <CarouselPrevious className="relative h-16 w-16 bg-white border-primary/20 text-primary hover:bg-primary hover:text-white transition-all shadow-xl rounded-full" />
              <CarouselNext className="relative h-16 w-16 bg-white border-primary/20 text-primary hover:bg-primary hover:text-white transition-all shadow-xl rounded-full" />
            </div>
          </Carousel>
        </div>
      </div>
      ...
      <Dialog open={!!selectedItem} onOpenChange={() => setSelectedItem(null)}>
        <DialogContent className="max-w-5xl p-0 overflow-hidden border-none rounded-[4rem] bg-background">
          {selectedItem && (
            <div className="grid grid-cols-1 lg:grid-cols-2">
              ...
              <div className="p-12 lg:p-20 space-y-10 max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-4 p-8 bg-secondary/20 rounded-[2.5rem] border border-primary/10 shadow-inner">
                  ...
                  <p className="text-foreground/80 leading-relaxed italic text-xl font-light">"{selectedItem.culturalStory}"</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-12">
                  <div className="space-y-6">
                    ...
                    <ul className="text-base text-foreground/60 space-y-2 font-light">
                ...
                <div className="pt-10 border-t border-muted flex flex-col sm:flex-row items-center justify-between gap-10">
      ...
      <Sheet>
        <SheetTrigger asChild>
          {cart.length > 0 && (
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="fixed bottom-12 right-12 z-50">
              <Button className="h-24 w-24 rounded-full bg-primary text-white shadow-[0_20px_50px_rgba(0,0,0,0.3)] border-4 border-white hover:scale-110 transition-all group">
                <ShoppingCart size={40} className="group-hover:rotate-12 transition-transform" />
                <span className="absolute -top-4 -right-4 bg-accent text-white w-10 h-10 rounded-full flex items-center justify-center text-sm font-black shadow-2xl ring-4 ring-background animate-bounce">
        ...
        <SheetContent className="w-full sm:max-w-lg bg-background border-l shadow-2xl p-0 rounded-l-[4rem]">
          <div className="flex flex-col h-full">
            <SheetHeader className="p-12 border-b bg-white/50 backdrop-blur-xl">
            ...
            <div className="flex-1 overflow-y-auto p-12 space-y-12">
              {cart.map(entry => (
                <div key={entry.item.id} className="flex gap-8 items-center group">
                  <div className="relative w-28 h-28 rounded-[2rem] overflow-hidden shadow-xl shrink-0 border border-muted">
                  ...
                  <div className="flex-1 min-w-0 space-y-2">
                    ...
                    <div className="flex items-center bg-secondary/30 rounded-2xl p-1.5 px-3 border border-primary/5 w-fit mt-4">
            ...
            <div className="p-12 bg-white/80 backdrop-blur-2xl border-t space-y-10 shadow-[0_-20px_50px_rgba(0,0,0,0.05)]">
              ...
              <Button onClick={sendWhatsAppOrder} className="w-full h-20 text-xl font-black uppercase tracking-widest bg-[#25D366] hover:bg-[#128C7E] text-white rounded-[2rem] shadow-2xl transition-all">
